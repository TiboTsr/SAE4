...
class Role extends BaseModel implements JsonSerializable
{
    ...
    {
        $data = $this->DB->select("SELECT " . self::selectClause() . " FROM ROLE WHERE id_role = ?", "i", [$this->id]);
    ...
    {
        $DB = new \DB();
        $result = $DB->select("SELECT " . self::selectClause() . " FROM ROLE");
    ...
    {
        $this->DB->query("INSERT INTO ASSIGNATION (id_membre, id_role) VALUES (?, ?)", "ii", [$member->getId(), $this->id]);
    }
